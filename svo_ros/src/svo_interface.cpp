...
void SvoInterface::globalpositionsCallback(const geometry_msgs::PoseStampedConstPtr& msg)
{
  ...
  if(globalpositions_handler_)
  {
    ...
    if(globalpositions_handler_->readInitialPose())
    {
      Eigen::Quaterniond orientation(1,0,0,0);

      // This can be used in the case that the initial world orientation is known.
      if(globalpositions_handler_->gp_settings_.initial_orientation_known)
      {
      ...
      if(globalpositions_handler_->setInitialPose(position, orientation))
      {
...
void SvoInterface::globalpositionsLoop()
{
  SVO_INFO_STREAM("SvoNode: Started Global Positions loop.");
  ros::NodeHandle nh;
  ros::CallbackQueue queue;
  nh.setCallbackQueue(&queue);
  std::string gp_topic = vk::param<std::string>(pnh_, "gp_topic", "gp");
  ros::Subscriber sub_gp =
      nh.subscribe(gp_topic, 5, &svo::SvoInterface::globalpositionsCallback, this);
  while(ros::ok() && !quit_)
  {
    queue.callAvailable(ros::WallDuration(0.1));
  }
